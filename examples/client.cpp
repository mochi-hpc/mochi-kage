...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Open the Proxy "myproxy" from provider 0
        kage::ProxyHandle proxy =
            client.makeProxyHandle(g_address, g_provider_id);

        int32_t result;
        proxy.computeSum(32, 54, &result);

    } catch(const kage::Exception& ex) {
...
void parse_command_line(int argc, char** argv) {
    try {
        TCLAP::CmdLine cmd("Kage client", ' ', "0.1");
